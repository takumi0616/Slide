...
- $x, w$：入力パターンとノード重み（$H \times W$ の格子場）
- $(s)=(i,j)$：内部格子点
- $\Omega_x$：右隣が格子内に存在する格子点の集合（$s=(i,j)$ に対し $(i,j+1)$ が格子内）
- $\Omega_y$：上隣が格子内に存在する格子点の集合（$s=(i,j)$ に対し $(i+1,j)$ が格子内）
- $\epsilon = 10^{-12}$：数値安定化の微小正値
...
- $\Omega^{\kappa}$：曲率が定義される**内部共通格子**上の格子点集合
  - 161×161 の格子から上下左右 2 マスである $H-4$，$W-4$ の格子へ縮退
...
| KAPPA     |   0 〜 1    |                     0.432 |                       0.503 |  <span class="cell-good"> 0.442</span> |
